			...
			<div class="browser-not-support">
				<div class="mdc-card card">
					...
					<div class="card-content" style="margin:16px 0px">				
						Something went wrong, Getting projecct !!
					</div>
				</div>
			</div>
		`;
...
function loadSignin() {	
	...
	/* input on focus out error messsages */
	document.querySelector(".mobile").addEventListener('focusout',(event)=> {
		let input_value = event.target.value;
		let error_type = "is-invalid"
		mobile_valid = validateMobile(input_value, error_type);
		setMobileValue(mobile_valid);
	});
	document.querySelector(".mobile").addEventListener('keyup',(event)=> {
		let input_value = event.target.value;
		let error_type = "is-hint";
		mobile_valid = validateMobile(input_value, error_type);
		setMobileValue(mobile_valid);
	});
	document.querySelector(".password").addEventListener('focusout',(event)=> {
		let input_value = event.target.value;
		let error_type = "is-invalid";
		password_valid = validateOtp(input_value, error_type);
		setPasswordValue(password_valid);
	});
	...
	function setMobileValue(mobiel_1) { mobile_valid = mobiel_1; }
	function setPasswordValue(password_1) { password_valid = password_1; }
}
...
function loadCoupon() {
	...
	var kyc_valid = false;
	floatingLabel();
	/* input field on focus out error messages */
	document.querySelector(".kyc").addEventListener('focusout',(event)=> {
		let input_value = event.target.value;
		let error_type = "is-invalid";
		kyc_valid = ValidateKyc(input_value, error_type);
		setKycidvalue(kyc_valid);
	});
	document.querySelector(".kyc").addEventListener('keyup',(event)=> {
		let input_value = event.target.value;
		let error_type = "is-hint";
		kyc_valid = ValidateKyc(input_value, error_type);
		setKycidvalue(kyc_valid);
	...
	document.querySelector(".coupon").addEventListener('focusout',(event)=> {	
		let input_value = event.target.value;
		let error_type = "is-invalid";
		coupon_valid = ValidateCoupon(input_value, error_type);
		setCouponCodeValue(coupon_valid);
	});
	...
	function setCouponCodeValue(coupon_1) { coupon_valid = coupon_1; }
	function setKycidvalue(kyc_1) { kyc_valid = kyc_1; }
}
...
*/
function validateMobile(Input_value, Error_type) {
	var inputFieldObj = {
		input_name : "mobile",
		input_value: Input_value,
		input_type: "numeric",
		input_error_element : "mobile-error",
		input_status : false,
		input_error_digits: "must contains 10 digits",
		input_pattern : /^([6-9][0-9]{9})$/, 
		input_error_msg_field: "Mobile Number",
		error_type : Error_type
	}
	let mobile_valid = inputErrorMsg(inputFieldObj);
	return mobile_valid;
}

/*
	This function is for the send required info for the mobile validation to the inputErrorMsg function.
	This is only for the OTP input field
*/
function validateOtp(Input_value, Error_type ) {
	var inputFieldObj = {
		input_name: "otp",
		input_value: Input_value,
		input_type: "numeric",
		input_error_element : "otp-error",
		input_status : false,
		input_error_digits: "must contains 6 digits",
		input_pattern : /^([0-9]{6})$/,
		input_error_msg_field: "OTP",
		error_type : Error_type
	}
	let otp_valid = inputErrorMsg(inputFieldObj);
	return otp_valid;
}

/*
	This function is for the send required info for the mobile validation to the inputErrorMsg function.
	This is only for the KYC input field
*/
function ValidateKyc(Input_value, Error_type) {
	var inputFieldObj = {
		input_name: "kyc",
		input_value: Input_value,
		input_type: "text",
		input_error_element : "kyc-error",
		input_status : false,
		input_error_digits: "is not valid",
		input_pattern : /^([0-9A-z\s]{7,9})$/,
		input_error_msg_field: "KYC",
		error_type : Error_type
	}
	let kyc_valid = inputErrorMsg(inputFieldObj);
	return kyc_valid;
}
...
*/
function ValidateCoupon(Input_value, Error_type){
	var inputFieldObj = {
		input_name: "coupon",
		input_value: Input_value,
		input_type: "text",
		input_error_element : "coupon-error",
		input_status : false,
		input_error_digits: "must contains 8 characters",
		input_pattern : /^([0-9A-z]{8})$/,
		input_error_msg_field: "Coupon Code",
		error_type : Error_type
	}
	let coupon_valid = inputErrorMsg(inputFieldObj);
	return coupon_valid;
}
...
function setHeader(logo_url,project_name, project_id) {
	let header =  document.querySelector("header");
	if(header != null) {
		header.innerHTML = `
			<div class="mdc-top-app-bar__row app-header">
				<img class="logo" loading="lazy" alt="Indira Gandhi International Airport" height="40px" width="auto">
			</div> 
		`;
...
function setFooter(project_id) {
	...
	if(footer != null) {
		footer.innerHTML = `
			<div class="mdc-top-app-bar__row footer">
				...
				<div class="footer-img">
					<img loading="lazy" src="assets/images/logo.webp" width="40px" />
				</div>
			</div>
			`
		...
		if(project_id == "igidel-free-wifi" || project_id ==  "goa-airport" ) {
...
async function showHideAdvBanners(top_adv_banner, main_adv_banner, banner_url, full_banner_url, project_id) {
...
function signinPageTabs(project_id) {
	if(project_id == "igidel-airindia-lounge" || project_id == "igidel-amex-lounge") {
...
function userHelpText(project_id) {
	...
	if(user_help_text_ele != null) {
		...
		else if(project_id == "igidel-airindia-lounge" || project_id == "igidel-amex-lounge") {
...
async  function loadPage(pageName = "") {
	...
	try {
		...
		setHeader(props.logo, props.project_name, props?.project_id);
		setFooter(props.project_id);
		setMainContainerHeight(props.project_id);
		termsConditions(props.project_id);
		userHelpText(props.project_id);
		faqContentInpage(props.faq_required, props.enable_faq_content, props.project_id);
		displayFaqlink(props.faq_required);
		showHideAdvBanners(props.enable_top_advertisement_banner, props.enable_main_advertisement_banner, props.banner, props.full_banner, props.project_id);
		logedinSessionTime();
		LoginNotes(props.project_id);
		signinPageTabs(props.project_id);
